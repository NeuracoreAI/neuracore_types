...
class NCData(BaseModel):
    ...
    Attributes:
        timestamp: Client-side timestamp (seconds since epoch) when the data
            was captured. Used for temporal synchronization. User specified
            or autogenerated if not provided.
        published_time: Timestamp (seconds since epoch) when the
            data point was received/published. Optional for backwards
            compatibility with data logged before this field was introduced.
            Used to break ties and detect out-of-order delivery.
    """
    ...
    def model_post_init(self, __context: Any) -> None:
        """Set timestamp to published_time if not explicitly provided.

        This ensures that if the user does not provide a timestamp, it will
        default to the published_time, which is the time the data was received.
        """
        if "timestamp" not in self.model_fields_set:
            object.__setattr__(self, "timestamp", self.published_time)
